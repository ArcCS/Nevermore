...
func (a *newCharacter) validateStats(statInput string) bool {
	...
	if config.RaceDefs[config.AvailableRaces[a.race]].DexMin > stats[1] || stats[1] > config.RaceDefs[config.AvailableRaces[a.race]].DexMax {
		return false

	}
	if config.RaceDefs[config.AvailableRaces[a.race]].ConMin > stats[2] || stats[2] > config.RaceDefs[config.AvailableRaces[a.race]].ConMax {
		return false
	}
	if config.RaceDefs[config.AvailableRaces[a.race]].IntMin > stats[3] || stats[3] > config.RaceDefs[config.AvailableRaces[a.race]].IntMax {
		return false
	}
	if config.RaceDefs[config.AvailableRaces[a.race]].PieMin > stats[4] || stats[4] > config.RaceDefs[config.AvailableRaces[a.race]].PieMax {
		return false
	}
	return true
