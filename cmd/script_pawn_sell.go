...
func (sell) process(s *state) {
	...
	if target != nil {
		...
		if s.actor.GetStat("int") < 5 {
			...
			finalValue = int((.10 * float64(s.actor.GetStat("int"))) * float64(target.Value))
		} else if s.actor.GetStat("int") >= 10 {
			finalValue = int((.5 * float64(target.Value)) +
				(45 / float64(s.actor.Int.Current) * (.25 * float64(target.Value))) +
				float64(utils.Roll(10, 1, 0)/10)*(.25*float64(target.Value)))
		}
