...
func (confirmName) process(s *state) {
	...
	if target != nil {
		...
		s.msg.Actor.SendGood("Please enter the name of your new item first with the name directive, do not use starter words like 'a' \nExample: name new sword. \n Or simply stop to cancel. \n !! You must stop NOW in order to not be charged. !!")
		s.actor.AddCommands("name", "$CONFIRMDESC "+targetStr+" "+strconv.Itoa(targetNum))
	} else {
		s.msg.Actor.SendInfo("What're you trying to rename??")
		return
	}
...
func (confirmDesc) process(s *state) {
	...
	if target != nil {
		...
		target.Name = newName
		s.msg.Actor.SendGood("Please enter the description of your item first with the 'description' directive \nExample: description it is very shiny. \n Or simply stop to cancel.")
		s.actor.AddCommands("description", "$CONFIRMRENAME "+strings.Split(newName, " ")[0]+" "+strconv.Itoa(targetNum)+" \""+newName+"\"")
	} else {
		s.msg.Actor.SendInfo("What're you trying to rename??")
		return
	}
...
func (confirmRename) process(s *state) {
	...
	newDescription := strings.Join(s.input[3:], " ")

	target := s.actor.Inventory.Search(targetStr, targetNum)

	if target != nil {
		s.msg.Actor.SendGood("The name of your item will be " + newName + ".")
		s.msg.Actor.SendGood("New Description:\n " + newDescription + ".")
		s.msg.Actor.SendGood("Type 'yes' if this how you would like your item to be renamed. Or simply stop to cancel.")
		s.actor.AddCommands("yes", "$COMPLETERENAME "+targetStr+" "+strconv.Itoa(targetNum)+" \""+newName+"\" "+"\""+newDescription+"\"")
	} else {
		s.msg.Actor.SendInfo("What're you trying to rename??")
		return
	}
...
type completeRename cmd

func (completeRename) process(s *state) {
	if len(s.words) < 3 {
		s.msg.Actor.SendInfo("Issue completing rename")
		return
	}
	...
	if target != nil {
		...
		s.msg.Actor.SendGood("Renamed " + target.Name + " to " + newName + " completed.")
	} else {
		s.msg.Actor.SendInfo("What're you trying to rename??")
		return
	}
